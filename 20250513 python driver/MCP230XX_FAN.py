...
import time, sys, smbus2 as smbus
...
class MCP230XX:
    ...
    def set_mode(self, pin, mode, pullUp='disable'):
        ...
        if mode == 'input':
            ...
            if pullUp == 'enable':
                ...
                regValue = regValue & mask                
                self.single_access_write(reg, regValue)            
        #print(hex(self.single_access_read(reg)))
        return
    ...
    def output(self, pin, value):
        ...
        reg, bit = self.register_bit_select(pin ,reg8A=0x09,reg16A=0X14,
        ...
        self.single_access_write(reg, regValue)              
#        print(hex(self.single_access_read(0x0A))) #debug
        return
...
if __name__ == "__main__":
    ...
    # Initialize MCP23008 on address 0x20
    mcp = MCP230XX('MCP23008', 0x21)

    fan_pin = 1  # Use GPIO4 of MCP23008 (adjust as needed)
    mcp.set_mode(fan_pin, 'input')  # Set as input
    mcp.invert_input(fan_pin, False)  # Optional: depends on logic level

    print("Monitoring fan tachometer pulses on GPIO4...")

    last_state = mcp.input(fan_pin)
    last_time = time.time()
    pulse_count = 0

    try:
        while True:
            current_state = mcp.input(fan_pin)
            ...
            if last_state == 1 and current_state == 0:
                ...
                rpm = 60.0 / (duration * 2)  # 2 pulses per rev
                print(f"Pulse {pulse_count}: Fan RPM ≈ {rpm:.2f}")
